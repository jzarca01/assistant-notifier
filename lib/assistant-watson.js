  ...
  init(plugins) {
    ...
    if (!this.apiKey || this.host) {
      return Promise.reject(
        '[assistant-watson] Erreur : vous devez configurer ce plugin !'
      );
    }
...
exports.init = (configuration, plugins) => {
  return new AssistantWatson(configuration).init(plugins).then(resource => {
    console.log('[assistant-watson] Plugin chargé et prêt.');
    return resource;
  });
};